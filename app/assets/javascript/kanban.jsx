	...
	render: function(){
		...
		return(
			<div className="task" style={style} >
				<div style={longitud}>{this.props.task} </div>
				<StatusSelect id={this.props.id} status={this.props.status} color={this.state.fondo}/>
				<div style={dibujo} onClick={this.deleteTask}>				
					<svg height='12pt' viewBox="0 0 214.15626 241.04342" display="block" >
						 <g  transform="translate(-312.23 -179.46)"  >
...
var StatusSelect = React.createClass({
	...
	render: function(){
		...
		return(				
			<div className="satusSelect" style={style} >			
				<select name="taskStatus" style={selectorStyle} ref="status">
					<option value="Pendiente" onClick={this.updateTask}> Pendiente </option>
					<option value="En camino" onClick={this.updateTask}> En camino </option>
					<option value="Acabado" onClick={this.updateTask}> Acabado </option>
				</select>
			</div>
			);
	}
...
var TaskList = React.createClass({
	render:function(){
		...
		var listadoTareas = this.props.data.map(function(tarea){
				...
				return(
					<Task task={tarea.task} color={fondo} text={texto} id={tarea._id} status={tarea.status} />);
			});
		var style = {backgroundColor:fondo, color:texto}
		return(
			<div className="taskList">
				<h1 style={style}> {this.props.lista}</h1>
				{listadoTareas}
			</div>
			);
	}
...
var TaskDashboard = React.createClass({
  ...
  componentDidMount: function() {
	...
	render: function(){
		...
		return(
			<div className="taskDashboard" style={style}>
				<TaskForm />
				<TaskList lista="Pendiente" data={this.state.pendientes} color="#FE2E2E" text="white"/>
				<TaskList lista="En camino" data={this.state.ongoing} color="#886A08"  text="white" />
				<TaskList lista="Acabado" data={this.state.acabados} color="#31B404" text="white" />
			</div>
			);
	}
});
	...
	render: function(){
		return(
			<form className="taskForm" onSubmit={this.addTask}>
				<input type="text"placeholder="Indica qué tarea quieres agregar" ref="tarea" />
				<select name="taskStatus" ref="status">
...
React.render(
	<TaskDashboard url="http://localhost:9000/tasks" pollInterval={1000} />,
